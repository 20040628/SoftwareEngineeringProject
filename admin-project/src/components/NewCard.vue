...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (!this.paymentData.bankCard) {
        this.errors.bankCard = 'Bank card number is required';
      } else if (!/^\d{16,19}$/.test(this.paymentData.bankCard)) {
        this.errors.bankCard = 'Bank card number must be between 16 and 19 digits';
      }
    ...
    async submitPayment() {
      ...
      try {
        ...
        const payload = {
          bankCard: this.paymentData.bankCard // 使用新的银行卡卡号字段
        };

        const res = await axios.post(`/api/bank-payment/newCard/${orderId}`, payload, {
          headers: {
