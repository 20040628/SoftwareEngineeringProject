...
public interface OrderRepository extends JpaRepository<Order, Long> {
    @Query("SELECT o FROM Order o WHERE o.scooter.id = :scooterId AND o.status IN (2, 3) AND o.startTime >= :startDate ORDER BY o.startTime")
    List<Order> findActiveOrdersByScooterId(@Param("scooterId") Long scooterId, @Param("startDate") Date startDate);

    @Query("SELECT o FROM Order o WHERE o.scooter.id = :scooterId AND o.status IN (2, 3) " +
    ...
    @Query("SELECT SUM(TIMESTAMPDIFF(HOUR, o.startTime, o.endTime)) FROM Order o " +
           "WHERE o.user.id = :userId AND o.status = 4 " +
    ...
    @Query("SELECT o FROM Order o WHERE o.user.id = :userId AND o.endTime < CURRENT_TIMESTAMP AND o.status = 4")
    ...
    @Query("SELECT SUM(o.price) FROM Order o " +
           "WHERE DATE(o.startTime) = DATE(:date) AND o.hirePeriod = 'HOUR' AND o.status = 4")
     ...
     */
    @Query("SELECT SUM(o.price) FROM Order o " +
           "WHERE DATE(o.startTime) = DATE(:date) AND o.hirePeriod = 'FOUR_HOURS' AND o.status = 4")
     ...
     */
    @Query("SELECT SUM(o.price) FROM Order o " +
           "WHERE DATE(o.startTime) = DATE(:date) AND o.hirePeriod = 'DAY' AND o.status = 4")
     ...
     */
    @Query("SELECT SUM(o.price) FROM Order o " +
           "WHERE DATE(o.startTime) = DATE(:date) AND o.hirePeriod = 'WEEK' AND o.status = 4")
     ...
     */
    @Query("SELECT SUM(o.price) FROM Order o " +
           "WHERE DATE(o.startTime) = DATE(:date) AND o.status = 4")
